...
public final class Paperclip {
    ...
    public static final Logger logger = new SimpleLogger("MorninggloryClip");

    public static void main(final String[] args) {
        if (Path.of("").toAbsolutePath().toString().contains("!")) {
            System.err.println("MorninggloryClip may not run in a directory containing '!'. Please rename the affected folder.");
            System.exit(1);
        }
    ...
    private static @NotNull DownloadContext getDownloadContextFromMojang(Path repoDir) {
        DownloadContext downloadContext = findDownloadContext(true);
